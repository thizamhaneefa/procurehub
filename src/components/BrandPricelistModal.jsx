...
import { UploadCloud, Trash2, ExternalLink, LayoutGrid, List, FileSpreadsheet } from "lucide-react";
...
export default function BrandPricelistModal({ brand, onClose }) {
  ...
  const load = () => {
    if (!brand) return;
    setLoading(true);
    fetch(`/api/brands/${brand.id}/pricelists`).then((r) => r.json()).then(setFiles).finally(() => setLoading(false));
  };
  useEffect(() => { if (brand) { setView("grid"); load(); } /* eslint-disable-next-line */ }, [brand?.id]);
  ...
  async function remove(f) {
    ...
    if (!ok) return;
    await fetch(`/api/brands/${brand.id}/pricelists/${f.id}`, { method: "DELETE" });
  ...
  return (
    <Modal open={open} onClose={onClose} xl title={brand ? `Pricelists — ${brand.name}` : "Pricelists"}>
      {brand && (
        <div className="space-y-4">
          {/* toolbar */}
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-slate-500">{files.length} file{files.length === 1 ? "" : "s"} · Excel only</p>
            <div className="flex items-center gap-2">
              ...
              <input ref={inputRef} type="file" accept=".xlsx,.xls,.csv" className="hidden" onChange={upload} />
            </div>
          </div>
          ...
          <div className="max-h-[60vh] min-h-[280px] overflow-y-auto rounded-xl border border-rose-100 bg-rose-50/40 p-3 dark:border-pink-950/50 dark:bg-[#1f0c16]/40">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-xs text-slate-400">Click “Upload Pricelist” to add an Excel file for this brand.</p>
              </div>
            ) : view === "grid" ? (
                ...
                {files.map((f) => (
                  <div key={f.id} className="group relative flex flex-col items-center rounded-xl border border-rose-100 bg-white p-3 text-center shadow-sm dark:border-pink-950/60 dark:bg-[#241019]">
                    <a href={f.fileUrl} target="_blank" rel="noreferrer" className="flex flex-1 flex-col items-center" title="Open">
                      <div className="mb-2 flex h-14 w-14 items-center justify-center rounded-lg bg-rose-50 dark:bg-pink-950/40"><FileSpreadsheet size={28} className="text-emerald-500" /></div>
